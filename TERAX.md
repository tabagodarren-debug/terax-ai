...
- **Afflow agent presets** - four fixed workflow roles are stored per workstation in Spaces schema v2, while CLI brands remain global launcher configuration. The presets surface supports Claude, Codex, Gemini, OpenCode, and validated custom commands; native detection reuses the login-shell PATH resolver. Role terminals always start at the authorized workstation root. Specialized prompt files are opened through canonical, containment-checked native resolution, startup instructions are copied instead of injected into a CLI TUI, and conservative terminal file references open authorized workstation files at an optional line number.
- **Afflow browser tools** - Spaces schema v3 stores per-workstation saved website launchers, profile mode, an opaque isolated profile ID, and optional auto-open. Global preferences own the preferred browser and validated executable overrides. Rust derives Chrome and Edge user-data directories under the app-local `browser-profiles` directory, launches URLs as separate process arguments, and never reads browser profile contents. Profile reset requires a matching native marker, an idle lock probe, explicit confirmation, and canonical non-link containment. Chrome and Edge detection supports Windows and macOS; activity uses Windows sharing semantics or the macOS Chromium `SingletonLock` advisory lock and fails closed on unsupported platforms. Cross-device configuration and file sync remain Phase 5 work. Browser profiles, cookies, secrets, CLI authentication, executable paths, and live process state must never be synchronized.
