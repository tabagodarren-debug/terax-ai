//! Windows-native mechanics for the browser module: the App Paths registry
//! lookup and the Chromium `lockfile` sharing probe.
//!
//! Only genuinely Win32 code lives here. Everything that merely *differs*
//! between platforms, including executable names, lock file names, candidate
//! order, and link-safe deletion, is in `platform.rs`, where it can be tested from any
//! host. See `chrome/browser/process_singleton_win.cc` upstream.
...
#[cfg(test)]
mod tests {
    use super::{probe_lock, read_app_paths};
    ...
    fn an_unheld_lockfile_probes_free_and_is_left_untouched() {
        ...
        assert_eq!(probe_lock(&lockfile), LockProbe::Free);
    ...
    fn a_lockfile_held_with_no_sharing_probes_busy() {
        ...
        let held = OpenOptions::new()
            ...
            .open(&lockfile)
            .unwrap();
        assert_eq!(probe_lock(&lockfile), LockProbe::Busy);
    ...
    fn an_unexpected_lockfile_entry_fails_closed_as_unknown() {
        ...
        assert_eq!(probe_lock(&lockfile), LockProbe::Unknown);
    }
